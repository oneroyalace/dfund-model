...
export default class extends Controller {
  ...
  getReportersPer() {
    return [...document.querySelectorAll(".numReportersInput")].map(x => parseInt(x.value))
                                                               .reduce((x,y) => x+y)
  }
  ...
  populateStateTable(){
    ...
    this.populationStateTableRowTarget.innerText = this.prettifyInteger(states[selectedState].population[this.getYear()])
  ...
  // Get multiplier value for a state
  getStateSizeMultiplier(activeState) {
    const stateSize = states[activeState]["size"]
    const multiplierMap = {
      "Extra Small": 0.5,
      "Small": 1,
      "Medium": 2,
  ...
  calculateEditorialEstimate() {
    ...
    const sizeMultiplier = this.getStateSizeMultiplier(activeState)

    return reportersPer * (
      (states[activeState].cd) + 
      (states[activeState].sd * sizeMultiplier) +
      (states[activeState].ts) +
      (states[activeState].muni) +
      (states[activeState].county * sizeMultiplier) +
      (states[activeState].state * sizeMultiplier) +
      (states[activeState].population[this.getActiveYear()]/100_000)) +

      (states[activeState].ss * sizeMultiplier)
  ...
  updateEstimates() {
    ...
    const editorialEstimate = this.calculateEditorialEstimate()
    const nonEditorialEstimate = editorialEstimate * (2/3)
    ...
    this.fundingEstimateTarget.innerText = this.prettifyInteger(Math.round(costPerEditorialEmployee * (editorialEstimate + nonEditorialEstimate)))
